...
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/eae11a10fe0d71b59e8ef.jpg"
else:
...
if lang == "si":
    ...
    async def inline_handler(event):
        ...
        if event.query.user_id == bot.uid and query.startswith("VirtualUserbot"):
            ...
            result = builder.article(
                title="Stats",
                text=f"**Showing Stats For {DEFAULTUSER}'s VirtualUserbot** \nNote --> Only Owner Can Check This \n(C) [VirtualUserbot](https://lasiya.ml )",
                buttons=[
                    [custom.Button.inline("Show Stats ?", data="terminator")],
                    [Button.url("Developed By", "https://lasiya.ml")],
                    [Button.url("Support Chat❤️", "t.me/Danuma01")],
                ],
            )
            await event.answer([result])
    ...
    async def rip(event):
        ...
        await borg.send_message(
            LOG_CHAT,
            f"ආයුබෝවන්, මෝඩ  [yaka](tg://user?id={him_id}) තහන්ම් එකක් තෝරපු නිසා Block කරා",
        )
        ...
        if event.query.user_id == bot.uid and query.startswith("VirtualUserbot"):
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        Button.url(
                            "Repo Here", "https://lasiya.ml "
                        )
                    ],
                    [Button.url("Join Channel ❤️", "t.me/D_bot_Ai")],
                ],
            )
            await event.answer([result])
        elif event.query.user_id == bot.uid and query.startswith("**Hello"):
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy VirtualUserbot To Get Your Own Assistant, Repo Link [Here](https://lasiya.ml )",
        )
